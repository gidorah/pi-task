...
from pi_task.db import RunRecord, get_run, open_db
...
def cancel_run(run_id: str) -> tuple[RunRecord, str]:
    ...
    does not cancel work; only this path targets an active unit.
    """
    # Local import avoids a circular dependency with runner.heal_orphaned_run.
    from pi_task.runner import heal_orphaned_run
    ...
    # systemctl stop is synchronous for oneshot services; the wrapper receives
    # SIGTERM, stops Pi with a grace period, and records cancelled. Allow enough
    # wall time for Pi's grace period plus finalization under load.
    _systemctl("stop", unit, timeout=60)
    with open_db() as connection:
